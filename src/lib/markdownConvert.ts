...
function renderMathInText(text: string, displayMode: boolean): string {
  try {
...
// Inline $...$ and display $$...$$ math (not inside code blocks).
marked.use({
  extensions: [
    {
      name: 'displayMath',
      ...
      renderer(token) {
        return renderMathInText((token as unknown as { text: string }).text, true);
      },
    },
    {
      name: 'inlineMath',
      ...
      renderer(token) {
        return renderMathInText((token as unknown as { text: string }).text, false);
      },
    },
  ],
});
